#!/usr/bin/env bash
# Backfill USAU MASTERS + GRAND MASTERS rosters for ONE season, slow & serial,
# operator-safe.
#
# Adapted from backfill-club-rosters.sh (identical safety model; only the
# competition_level filter differs). Masters events are ingested from ultirzr
# (ingest-from-ultirzr division='masters'), which leaves usau_event_team_url_id
# null — resolve-event-team-urls (masters-aware since 2026-07) fills those from
# the Masters-/Grand-Masters-/Great-Grand-Masters- schedule pages, then
# sync-event-rosters scrapes each team page. Same safety model — this hits USA
...
LOG="/tmp/masters-roster-backfill-${YEAR}-$(date +%Y%m%d-%H%M%S).log"
consec_fail=0
total_teams=0
total_players=0
total_events=0
...
fetch_events() {
  curl -s "$REST/usau_events?select=usau_slug,start_date,usau_games!inner(id)&competition_level=in.(MASTERS,GRAND_MASTERS)&season=eq.$YEAR&order=start_date.asc&usau_games.limit=1" \
...
say "=== MASTERS+GM roster backfill — season $YEAR ==="
...
say "Masters/GM events with games in $YEAR: $EVENT_COUNT"
