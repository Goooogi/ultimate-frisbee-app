...
// The CTA "Build your team" links to /fantasy/ufa/team — auth is deferred
// until the user attempts a WRITE action inside the builder.
...
export default async function UfaGameHomePage() {
  ...
  return (
    ...
    >
      ...
      <YourLeagues
        globalPool={globalContest ? { name: `UFA ${globalContest.seasonYear} · Public League` } : null}
      />
            ...
            <ol aria-label="Fantasy leaderboard">
              {leaderboard.map((row, idx) => {
                ...
                return (
                  <li key={row.teamId}>
                    <Link
                      href={`/fantasy/ufa/team/${row.teamId}`}
                      className={[
