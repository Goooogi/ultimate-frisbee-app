...
function SaveButton({
  ...
  return (
    <button
      ...
      aria-label="Save"
      className={[
        'inline-flex items-center justify-center gap-1.5 px-4 py-2 rounded-md min-h-[44px] min-w-[72px]',
        ...
        disabled || isSaving
          ? 'bg-[rgb(var(--ink)/0.06)] text-faint cursor-not-allowed'
      ...
      <div className="flex gap-2 items-start">
        <div className="flex-1">
          <input
            ...
            spellCheck={false}
            className={[
              'w-full bg-surface border px-3 py-2.5 text-[14px] font-semibold text-ink font-tight rounded',
              'focus-visible:outline-none transition-colors min-h-[44px]',
              tooLong || (profanityErr && trimmed)
                ? 'border-[rgb(var(--live))] focus-visible:border-[rgb(var(--live))]'
          ...
          <div className="flex items-center justify-between mt-1">
            ...
            <span
              className={[
                ...
                charCount > 55 ? 'text-[rgb(var(--live))]' : 'text-faint',
              ].join(' ')}
            >
              {charCount}/60
            </span>
          ...
          {profanityErr && trimmed && status !== 'error' && (
            <p className="text-[11px] font-tight text-[rgb(var(--live))] mt-1">
              {profanityErr}
            </p>
          )}
        </div>
...
function HandleField({ initial }: { initial: string | null }) {
  ...
  const borderClass =
    checkStatus === 'ok' || checkStatus === 'unchanged'
      ? 'border-[#22c55e] focus-visible:border-[#22c55e]'
        ...
        <div className="flex-1">
          <div className="relative flex items-center">
            ...
            <input
              ...
              className={[
                ...
                'focus-visible:outline-none transition-colors min-h-[44px] border',
                borderClass,
              ].join(' ')}
            />
            {/* Inline status icon */}
          ...
          <div className="mt-1.5 text-[11px] font-tight min-h-[16px]">
            ...
            {saveStatus === 'error' && saveError && (
              <span role="alert" className="text-[rgb(var(--live))]">{saveError}</span>
            )}
            {saveStatus !== 'saved' && saveStatus !== 'error' && (
              <>
                ...
                {checkStatus === 'taken' && <span className="text-[rgb(var(--live))]">That handle is already taken</span>}
                {checkStatus === 'format' && <span className="text-[rgb(var(--live))]">3–30 chars · lowercase letters, numbers, underscores</span>}
                {checkStatus === 'profanity' && <span className="text-[rgb(var(--live))]">Handle contains language that isn&apos;t allowed</span>}
