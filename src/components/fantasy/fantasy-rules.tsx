// Fantasy rules + scoring card. Presentational Server Component (reads the
// SCORING constant only). Shown on the My Team page so the leaderboard stays
// purely the leaderboard. `showCta` renders the "Build your team" button
// (used where the card isn't already on the builder itself).
...
export function FantasyRules({ showCta = false }: FantasyRulesProps) {
  const rows: { stat: string; off: string; def: string; neg: boolean }[] = [
