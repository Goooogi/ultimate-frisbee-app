 ...
 *
 * Shared between the CLI backfill (Phase 1) and the future scheduled edge
 * function (Phase 2). Pure/portable: no dotenv, no process.exit, no Node-only
 * imports. The only I/O is whatever `fetchHtml` the caller injects — keeps the
 * parse logic testable and edge-safe.
 *
