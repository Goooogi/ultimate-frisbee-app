-- Backfill USAU COLLEGE team state from the school name. College teams' venue
-- data was too sparse for the venue-modal derivation, but the name IS the
-- school, so state is unambiguous. Matches by base name (stripping " (B)"/"(C)"
-- second-team suffixes → same school, same state). Durable: schools don't move.
-- Only fills rows still missing a state (won't touch club overrides).
...
WHERE t.competition_level::text LIKE 'COLLEGE%'
  ...
  -- strip trailing " (B)"/" (C)" second-team markers, collapse double spaces
